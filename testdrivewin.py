#!/usr/bin/env python3
"""PyQt6 Windows app for Wheeltec robot parameter control and deployment."""
...
class SimOutputWorker(QThread):
    """Runs run_sim.bat in a background thread and streams its output."""
...
class SimInitiationDialog(QDialog):
    ...
    def _stop(self):
        ...
        try:
            open(os.path.join(tempfile.gettempdir(), 'movement_pause'), 'w').close()
        except Exception:
            pass
...
class CondaInstallWorker(QThread):
    """Downloads Miniforge3 for Windows, installs it, then runs
    setup_robostack_windows.bat to create the ros_env conda environment
    with ROS 2 Humble."""
    output   = pyqtSignal(str)
    ...
    def run(self):
        ...
        try:
            self.output.emit("[1/3] Downloading Miniforge3 for Windows x86_64...")
        ...
        # ── Step 3: Run setup_robostack_windows.bat to create ros_env ────
        setup_script = os.path.join(_PKG_DIR, "setup_robostack_windows.bat")
        if not os.path.isfile(setup_script):
            self.output.emit(f"ERROR: setup_robostack_windows.bat not found at {setup_script}")
            self.finished.emit(False)
            return
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                self.output.emit(f"\nsetup_robostack_windows.bat failed (exit {proc.returncode}).")
                self.finished.emit(False)
        except Exception as e:
...
class RobotControlApp(QMainWindow):
    ...
    def _build_node_canvas_tab(self):
        ...
        self.canvas_pkg_label.setFont(QFont("Consolas", 26, QFont.Weight.Bold))
        ...
        canvas_tree_lbl.setFont(QFont("Consolas", 11, QFont.Weight.Bold))
        ...
        self.canvas_file_tree.setFont(QFont("Consolas", 11))
    ...
    def _populate_canvas_file_tree(self):
        ...
        for pkg_name, pkg_info in self._canvas_state.packages.items():
            ...
            pkg_item.setFont(0, QFont("Consolas", 11, QFont.Weight.Bold))
            ...
            nodes_header.setFont(0, QFont("Consolas", 10, QFont.Weight.Bold))
            ...
            topics_header.setFont(0, QFont("Consolas", 10, QFont.Weight.Bold))
    ...
    def _build_code_editor_tab(self):
        ...
        self.simple_editor.setFont(QFont("Consolas", 13))
        ...
        fv_tree_lbl.setFont(QFont("Consolas", 11, QFont.Weight.Bold))
        fv_tree_lbl.setContentsMargins(4, 2, 4, 2)
        fv_left_layout.addWidget(fv_tree_lbl)

        self.file_tree = QTreeWidget()
        self.file_tree.setHeaderHidden(True)
        self.file_tree.setFont(QFont("Consolas", 11))
        ...
        self.full_editor.setFont(QFont("Consolas", 12))
    ...
    def _build_roboapps_tab(self):
        ...
        roboapps_label.setFont(QFont("Consolas", 13, QFont.Weight.Bold))
    ...
    def _is_conda_installed(self):
        ...
        candidates = [
            ...
            os.path.expanduser("~/miniforge3/condabin/conda.bat"),
    ...
    def _on_conda_icon_clicked(self):
        ...
        if conda_prefix:
            ...
            msg = (
                ...
                "Run  setup_robostack_windows.bat  in the project folder to set it up."
            )
    ...
    def _show_conda_not_installed_dialog(self):
        ...
        dlg.setText("ROS 2 Humble in a Conda environment is required to run Rviz or Gazebo on your Windows PC.")
    ...
    def _load_file_tree(self):
        ...
        def _ensure_folder(dir_name):
            ...
            folder_item.setFont(0, QFont("Consolas", 11, QFont.Weight.Bold))
    ...
    def _find_conda_env(self):
        ...
        try:
            ...
            for env_path in envs:
                if env_path.endswith("\\ros_env") or env_path.endswith("/ros_env"):
    ...
    def _generate_fastdds_xml(self, robot_ip):
        ...
        and its VOLATILE topics (/tf, /odom, /scan ...) actually flow to the Windows PC.
        """
        locators = "\n".join(
            f"          <locator><udpv4>"
...
"""
    ...
    def _show_sim_initiation_dialog(self):
        ...
        script_path = os.path.join(_PKG_DIR, "run_sim.bat")
        if not os.path.isfile(script_path):
            self._log("WARNING: run_sim.bat not found in project folder.")
            return
